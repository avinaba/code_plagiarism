...
Sig *	signature(FILE *);
int	compare(Sig *, Sig *);
int in_range(unsigned long int , unsigned long int);
...
int compare(Sig *s0, Sig *s1)
{
	...
	while (i0 < s0->nval && i1 < s1->nval) {
		if (in_range(s0->val[i0], s1->val[i1])) {
			v = s0->val[i0];
...
int in_range(unsigned long int number1, unsigned long int number2)
{
	if(abs(number1 - number2) < 1*(number1+number2))
		return 1;
	return 0;
}
